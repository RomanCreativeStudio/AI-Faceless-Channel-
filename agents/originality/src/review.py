"""Derives one ORIGINALITY_REVIEW verdict from the eight signal
evaluations, per CONTRACT.md's "Verdict derivation" list. No signal here
ever reaches REJECT (REJECT_TIER_SIGNALS is empty) — see CONTRACT.md's
Core principle: a definitive duplication/plagiarism judgment is a human
call, never this system's. REJECT is reserved for structural failures,
raised separately in pipeline.py.
"""
...
def derive_verdict(
    ...
    for e in evaluations:
        if e.risk_level is RiskLevel.HIGH_RISK:
            ...
            escalate = True
            if e.signal in REJECT_TIER_SIGNALS:  # always empty for this agent
                verdict = ReviewVerdict.REJECT
            elif verdict is not ReviewVerdict.REJECT:
    ...
    if verdict is ReviewVerdict.PASS:
        reasons.append(
            f"all {len(evaluations)} originality signals are LOW_RISK or NOT_APPLICABLE"
        )
