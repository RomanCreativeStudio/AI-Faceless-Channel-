"""Tests 16-17 from the task: dry-run makes no mutations; apply mode
changes only permitted fields."""
import tempfile
import unittest
...
class ApplyModeTests(unittest.TestCase):
    ...
    # --- Test 16: dry-run makes no mutations ---
    ...
    def test_dry_run_against_real_golden_sample_never_writes(self):
        ...
        run_originality_review(GOLDEN_SAMPLE, apply=False)
        after = (GOLDEN_SAMPLE / "CONTENT_ITEM.md").read_text(encoding="utf-8")
        self.assertEqual(before, after)
        self.assertFalse((GOLDEN_SAMPLE / "reviews").exists())

    # --- Test 17: apply mode changes only permitted fields ---
    def test_apply_writes_review_and_only_originality_state(self):
        ...
        self.assertIn(f"| Originality state | `{result.verdict.value}` |", after)
        self.assertIn("| Research state | `COMPLETE` |", after)
        self.assertIn("| Fact-check state | `NOT_STARTED` |", after)
        self.assertIn("| Safety state | `NOT_STARTED` |", after)
        self.assertIn("| Owner approval state | `NOT_STARTED` |", after)

    def test_apply_never_writes_a_claims_or_research_file(self):
        claims_before = {
            p: p.read_text(encoding="utf-8") for p in (self.root / "claims").glob("*.md")
        }
        run_originality_review(self.root, apply=True, channel_index=[])
        claims_after = {
            p: p.read_text(encoding="utf-8") for p in (self.root / "claims").glob("*.md")
        }
